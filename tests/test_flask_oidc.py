...
client_secrets_fp = Path(__file__).parent / "client_secrets.json"

client_secrets = json.loads(client_secrets_fp.read_text())
...
class MockHttp:
    def request(self, path, method="GET", post_string="", **kwargs):
        ...
        if path == "https://test/token":
            ...
            assert post_args["client_id"] == ["MyClient"], "Client ID is specified"
...
def test_signin(make_test_client, mocker):
    ...
    r1 = test_client.get("/")
    assert r1.status_code == 302, "Expected redirect to IdP " "(response status was {response.status})".format(
        response=r1
    )
    ...
    assert r2.status_code == 302, "Expected redirect to destination " "(response status was {response.status})".format(
        response=r2
    )
    r2location = urlsplit(r2.headers["Location"])
    assert r2location.path == "/", "Expected redirect to destination " "(unexpected path {location.path})".format(
        location=r2location
    )
...
def test_refresh(make_test_client, mocker):
    ...
    mocker.patch("time.time").return_value = current_time + 10
...
def test_validate_token_return_false(make_test_client, mocker):
    ...
    # flask providers - flask oidc
    from tests.app import oidc

    no_token_err = oidc.validate_token(None)
    assert bool(no_token_err) is False, "Expected no_token_err to eval to False"
    assert no_token_err == "Token required but invalid", "Expected correct no token error message"
